...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'ubah_password') {
        ...
        $upd  = $conn->prepare("UPDATE guru SET password = ? WHERE id = ?");
        $upd->bind_param("si", $hash, $guru_id);
        if ($upd->execute()) {
    ...
    <div class="flex-1 ml-64 overflow-y-auto pb-16">
        <div class="bg-white shadow-sm px-6 py-4 flex items-center justify-between sticky top-0 z-10">
            <div>
                ...
                <p class="text-gray-500 text-sm">Ganti password akun guru Anda</p>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="relative">
                            <input type="password" id="password-lama" placeholder="Password lama"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 pr-10">
                    ...
                    <button onclick="ubahPassword()"
                        class="w-full bg-green-700 hover:bg-green-800 text-white font-semibold py-2.5 rounded-lg transition-colors flex items-center justify-center gap-2">
...
function ubahPassword() {
    ...
    fetch('/guru/ubah_password.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(res => {
