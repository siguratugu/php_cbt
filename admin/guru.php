...
<head>
    ...
    <title>Data Guru - CBT MTsN 1 Mesuji</title>
        ...
        <div class="bg-white shadow-sm px-6 py-4 flex items-center justify-between sticky top-0 z-10">
            ...
            <div class="flex items-center gap-2">
                ...
                <button onclick="openModalImport()" class="bg-emerald-600 hover:bg-emerald-700 text-white text-sm px-4 py-2 rounded-lg flex items-center gap-1 transition-colors">
                    <i data-lucide="upload" class="w-4 h-4"></i> Import Guru
                </button>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-200">
                                ...
                                <th class="px-4 py-3 text-left text-gray-600 font-semibold">NIK</th>
...
<div id="modal-import" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 class="text-lg font-semibold text-gray-800">Import Guru</h2>
        ...
        <div class="p-6 space-y-4">
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-blue-600 mt-1">Download template terlebih dahulu, isi data guru, lalu upload file xlsx.</p>
                </div>
            </div>
            <a href="/template/template_guru.php"
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
            ...
            <button onclick="importGuru()" class="bg-blue-700 hover:bg-blue-800 text-white px-5 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
...
<div id="modal-tambah" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 class="text-lg font-semibold text-gray-800">Tambah Guru</h2>
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
            ...
            <button onclick="saveGuru()" class="bg-blue-700 hover:bg-blue-800 text-white px-5 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
...
<div id="modal-edit" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">NIK <span class="text-red-500">*</span></label>
                <input type="text" id="edit-nik" maxlength="16" inputmode="numeric"
                    oninput="this.value=this.value.replace(/\D/g,'')"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 font-mono">
                <p class="text-xs text-gray-400 mt-1">Harus 16 digit angka</p>
            </div>
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
            ...
            <button onclick="updateGuru()" class="bg-blue-700 hover:bg-blue-800 text-white px-5 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
...
function loadData() {
    ...
    const url = `/admin/ajax/guru_handler.php?action=get_all&page=${currentPage}&per_page=${perPage}`;
    fetch(url)
        .then(r => r.json())
        .then(res => {
            if (res.status === 'success') renderTable(res.data, res.total, res.per_page, res.page);
            else document.getElementById('table-body').innerHTML = '<tr><td colspan="5" class="text-center py-10 text-red-400">Gagal memuat data</td></tr>';
        });
}
...
function renderTable(data, total, perPage, page) {
    ...
    tbody.innerHTML = data.map((row, i) => `
        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
            ...
            <td class="px-4 py-3 font-mono text-gray-600">${escHtml(row.nik)}</td>
            <td class="px-4 py-3">
                <div class="flex items-center gap-1.5">
                    ...
                    <button onclick="openModalEdit(${row.id}, '${escHtml(row.nama).replace(/'/g,"&#39;")}', '${escHtml(row.nik)}')"
                    ...
                    <button onclick="deleteGuru(${row.id}, '${escHtml(row.nama).replace(/'/g,"&#39;")}')"
...
function saveGuru() {
    ...
    const nik      = document.getElementById('tambah-nik').value.trim();
    ...
    if (!/^\d{16}$/.test(nik))      { Swal.fire({ icon: 'warning', text: 'NIK harus 16 digit angka', confirmButtonColor: '#1d4ed8' }); return; }
    ...
    fetch('/admin/ajax/guru_handler.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(res => {
...
// ── Modal Edit ────────────────────────────────────────────────────────────────
function openModalEdit(id, nama, nik) {
...
function resetPasswordTerpilih() {
    ...
    Swal.fire({
        ...
        html: `Password <strong>${selectedIds.length} guru</strong> akan direset ke <code>123456</code>`,
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#d97706',
        cancelButtonColor: '#6b7280',
