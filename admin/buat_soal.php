...
if (!$bank_soal_id) {
    header('Location: /admin/bank_soal.php');
    exit;
}
...
<!-- ==================== TOP HEADER BAR ==================== -->
<header class="bg-blue-700 text-white px-5 py-3 flex items-center justify-between shadow-lg flex-shrink-0 z-20">
    <div class="flex items-center gap-4 min-w-0">
        <a href="/admin/bank_soal.php"
            class="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-500 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex-shrink-0">
    ...
    <div class="flex items-center gap-2 flex-shrink-0">
        <span id="total-count-badge"
            class="bg-blue-600 text-white text-xs font-semibold px-3 py-1.5 rounded-full flex items-center gap-1.5">
            <i data-lucide="list-ordered" class="w-3.5 h-3.5"></i>
            <span id="total-count-text">0 soal</span>
        </span>
        <button onclick="simpanKeBank()"
            class="bg-white text-blue-700 hover:bg-blue-50 text-sm font-semibold px-4 py-1.5 rounded-lg flex items-center gap-2 transition-colors shadow">
        ...
        <div class="w-56 xl:w-64 bg-white border-r border-gray-200 flex flex-col flex-shrink-0 shadow-sm">
            ...
            <div class="p-3 border-t border-gray-100 bg-gray-50 space-y-1.5">
                <div class="flex items-center gap-2 text-xs text-gray-500">
                    <span class="w-4 h-4 rounded bg-blue-600 inline-block flex-shrink-0"></span> Aktif
                </div>
                <div class="flex items-center gap-2 text-xs text-gray-500">
            ...
            <div id="empty-state" class="flex-1 flex items-center justify-center text-center p-8">
                <div class="max-w-sm">
                    ...
                    <button onclick="addNewSoal()"
                        class="bg-blue-700 hover:bg-blue-800 text-white px-5 py-2 rounded-xl text-sm font-medium inline-flex items-center gap-2 transition-colors">
                ...
                <div class="bg-white border-b border-gray-200 px-5 py-3 flex items-center justify-between flex-shrink-0 shadow-sm">
                    <div class="flex items-center gap-3">
                        <span id="soal-number-display"
                            class="bg-blue-700 text-white text-sm font-bold px-3 py-1 rounded-lg min-w-[72px] text-center">
                            Soal #1
                        </span>
                        <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button onclick="saveSoal()"
                            class="flex items-center gap-1.5 bg-blue-700 hover:bg-blue-800 text-white px-4 py-1.5 rounded-lg text-sm font-medium transition-colors shadow-sm">
            ...
            <div class="bg-white rounded-2xl shadow-sm p-8 border border-gray-200">
                ...
                <div class="mb-5 p-4 bg-blue-50 rounded-xl border border-blue-200 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-700 mt-1">Download template di bawah, isi soal sesuai format yang tersedia, lalu upload kembali.</p>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <button onclick="importWord()"
                        class="w-full bg-blue-700 hover:bg-blue-800 text-white py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-colors">
...
async function loadSoalList() {
    try {
        const res  = await fetch(`/admin/ajax/soal_handler.php?action=get_soal&bank_soal_id=${BANK_SOAL_ID}`);
...
async function saveSoal() {
    ...
    try {
        const res  = await fetch('/admin/ajax/soal_handler.php', { method: 'POST', body: fd });
        const data = await res.json();
...
async function simpanKeBank() {
    ...
    }).then(r => { if (r.isConfirmed) window.location.href = '/admin/bank_soal.php'; });
}
...
function importWord() {
    ...
    fetch('/admin/ajax/soal_handler.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(res => {
